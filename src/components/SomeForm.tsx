...
export function SomeForm() {
  const validationService = useInjection<IValidationService>(TYPES.ValidationService);
  ...
  const handleSubmit = async (data: unknown) => {
    ...
    try {
      const validatedData = await validationService.validateObject(schema, data);
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); handleSubmit({ name: 'John', email: 'john@example.com', age: 25 }); }}>
